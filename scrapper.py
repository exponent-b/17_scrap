...
def saramin_incruit(keyword, page):
    ...
    for i in range(page):
        ...
        # re = requests.get(f"https://www.saramin.co.kr/zf_user/search/recruit?searchword={keyword}&go=&flag=n&searchMode=1&searchType=search&search_done=y&search_optional_item=n&recruitPage={page}", headers=headers) # GET 요청을 보내서 해당 URL의 응답(Response)을 받아오는 함수
        re = requests.get(f"https://www.saramin.co.kr/zf_user/search/recruit?searchword={keyword}&go=&flag=n&searchMode=1&searchType=search&search_done=y&search_optional_item=n&recruitPage={page}&recruitSort=relation&recruitPageCount=30", headers=headers) # GET 요청을 보내서 해당 URL의 응답(Response)을 받아오는 함수
